...
class Trainer(object):
    ...
    def __init__(self, name, model, batch_size, learning_rate, step_size, weight_decay, data_dir, ngpus, workers, use_scheduler, 
                 task=None, max_samples=None, shuffle=True, data_subdir='train', includePaths=False, optim='adam', 
    ...
    def __call__(self, x, y, update_weights=True, activations=False, gradients=False):
        self.model.train()
...
class Validator(object):
    ...
    def __call__(self, x, y, activations=False, gradients=False):
        if self.ngpus > 0:
            y = y.cuda(non_blocking=True)
            x = x.cuda(non_blocking=True)
        if self.task is None:
...
def train_split(config_file, pretrained=False, restore_epoch=-1,
    ...
    # Get Model
    config = Config(config_file=config_file)
    model, ckpt_data = config.get_model(ngpus=ngpus, pretrained=pretrained, epoch=restore_epoch)
    ...
    if pretrained:
        ...
        print('Succesfully restored model from epoch ' + str(start_epoch-1))
        print()
    else:
        start_epoch = 1
        walltime = 0.0
    ...
    valid_num_steps = max(len(validator1.data_loader), len(validator1.data_loader))
    valid_step_freq = int(train_num_steps*valid_freq) 
    stop_epoch = start_epoch + epochs
    for epoch in trange(start_epoch, stop_epoch+1, initial=start_epoch, desc='epoch'):
        #for train_step, train_batch in enumerate(tqdm(trainer.data_loader, desc='train')):
            
        for train_step, (train_batch1, train_batch2) in enumerate(tqdm(itertools.zip_longest(trainer1.data_loader, 
                                                                              trainer2.data_loader, 
                                                                              fillvalue=None), desc='train')):
            global_step = (epoch-1) * train_num_steps + train_step
            '''
            # validate epoch
            if ((train_step) % valid_step_freq) == 0:
                ...
                with torch.no_grad():
                    ...
                    for valid_step, valid_batch in enumerate(tqdm(validator1.data_loader, desc=validator1.name)):
                        x_valid, y_valid = valid_batch
                        valid_start_time = time.time()
                        valid_loss, y_pred_valid = validator1(x=x_valid,y=y_valid)
                        valid_sec_per_iter = time.time() - valid_start_time
                        y_prob_valid, y_pred_valid = torch.topk(input=y_pred_valid.data,k=1)
                        y_valid, y_pred_valid = y_valid.cpu().numpy(), y_pred_valid.cpu().numpy()
                        valid_accuracy = metrics.accuracy_score(y_true=y_valid, y_pred=y_pred_valid)
                        avg_valid_loss+=valid_loss
                        avg_valid_accuracy+=valid_accuracy
                    avg_valid_loss/=valid_num_steps
                    avg_valid_accuracy/=valid_num_steps

                    writer.add_scalar('valid/loss1',     avg_valid_loss,     global_step)
                    writer.add_scalar('valid/accuracy1', avg_valid_accuracy, global_step)
                # task 2    
                ...
                with torch.no_grad():
                    ...
                    for valid_step, valid_batch in enumerate(tqdm(validator2.data_loader, desc=validator2.name)):
                        x_valid, y_valid = valid_batch
                        valid_start_time = time.time()
                        valid_loss, y_pred_valid = validator2(x=x_valid,y=y_valid)
                        valid_sec_per_iter = time.time() - valid_start_time
                        y_prob_valid, y_pred_valid = torch.topk(input=y_pred_valid.data,k=1)
                        y_valid, y_pred_valid = y_valid.cpu().numpy(), y_pred_valid.cpu().numpy()
                        valid_accuracy = metrics.accuracy_score(y_true=y_valid, y_pred=y_pred_valid)
            ...
            if train_batch1 is not None:
                ...
                train_loss1, _  = trainer1(x=x_train1, y=y_train1) #y_pred_train1
                #y_prob_train1, y_pred_train1 = torch.topk(input=y_pred_train1.data,k=1)
                #y_train1, y_pred_train1 = y_train1.detach().cpu().numpy(), y_pred_train1.detach().cpu().numpy()
                #train_accuracy1 = metrics.accuracy_score(y_true=y_train1, y_pred=y_pred_train1)
                writer.add_scalar('train/loss1',     train_loss1,     global_step)
                #writer.add_scalar('train/accuracy1', train_accuracy1, global_step)
            
            if train_batch2 is not None:
                x_train2, y_train2 = train_batch2
                train_loss2, _ = trainer2(x=x_train2, y=y_train2) #y_pred_train2
                #y_prob_train2, y_pred_train2 = torch.topk(input=y_pred_train2.data,k=1)
                #y_train2, y_pred_train2 = y_train2.detach().cpu().numpy(), y_pred_train2.detach().cpu().numpy()
                #train_accuracy2 = metrics.accuracy_score(y_true=y_train2, y_pred=y_pred_train2)
                writer.add_scalar('train/loss2',     train_loss2,     global_step)
                #writer.add_scalar('train/accuracy2', train_accuracy2, global_step)
        ...
        if ((epoch % save_freq) == 0):
            ckpt_data = {}
            ckpt_data['batch_size']     = config.batch_size
            ckpt_data['learning_rate']  = config.learning_rate
            ckpt_data['momentum']       = config.momentum
            ckpt_data['step_size']      = config.step_size
            ckpt_data['weight_decay']   = config.weight_decay
            ckpt_data['walltime']       = walltime
    ...
    writer.close()    
'''    
def train_split(config_file, split_index, pretrained=False, restore_epoch=-1,
    ...
    print('max_train_samples:', config.max_train_samples)
    print('max_valid_samples:', config.max_valid_samples)
    print('ngpus:            ', ngpus)
    print('workers:          ', workers)
    print('restore_epoch:    ', restore_epoch)
    print('epochs:           ', epochs        )
            ...
            if ((train_step) % valid_step_freq) == 0:
                ...
                with torch.no_grad():
                    ...
                    for valid_step, valid_batch in enumerate(tqdm(validator.data_loader, desc='valid')):
                        x_valid, y_valid = valid_batch
                        valid_start_time = time.time()
                        valid_loss, y_pred_valid = validator(x=x_valid,y=y_valid)
                        valid_sec_per_iter = time.time() - valid_start_time
                        y_prob_valid, y_pred_valid = torch.topk(input=y_pred_valid.data,k=1)
                        y_valid, y_pred_valid = y_valid.cpu().numpy(), y_pred_valid.cpu().numpy()
                        valid_accuracy = metrics.accuracy_score(y_true=y_valid, y_pred=y_pred_valid)
                    ...
                    writer.add_scalar('valid/loss',     avg_valid_loss,     global_step)
                    writer.add_scalar('valid/accuracy', avg_valid_accuracy, global_step)
            
            # train step
            x_train, y_train = train_batch
            train_start_time = time.time()
            train_loss, y_pred_train = trainer(x=x_train, y=y_train)
            train_sec_per_iter = time.time() - train_start_time
            y_prob_train, y_pred_train = torch.topk(input=y_pred_train.data,k=1)
            y_train, y_pred_train = y_train.cpu().numpy(), y_pred_train.cpu().numpy()
...
def predict(model, data_loader, ngpus, task=None, topk=1, notebook=False, max_batches=None, reduce_loss=True, 
            seed=None, reduction='mean'):
    '''
        ...
        for step, batch in enumerate(data_loader):
            ...
            if reduction=='none':
                if step == 0:
                    ...
                    losses = np.concatenate((losses, loss.cpu().numpy()), axis=0)
            ...
            #print('y_trye_all.shape:', y_true_all.shape)
...
def gradients_and_activations(activations=False, gradients=False, max_batches=None, validator=None, trainer=None, notebook=False):
    ...
    for step, batch in enumerate(tqdm(data_loader, desc='act/grad')):
        ...
        if trainer is not None:
            ...
            assert(len(all_activations[step]) == len(all_gradients[step])), "Something is wrong with activations or gradients, tell Julio lol"
...
def get_subgroup_performances(subgroups_file, y_true, y_pred, losses):
    '''
    Description: returns a dictionary of the predictions, labels, losses, average loss, and accruacy of all subgroups
    Note: Will not work if there are classes in subgroup_file that are not in y_true and vice versa. 
    '''
    categ2subgroup = np.load(subgroups_file)
    subgroups = {}

    subgroup_ids = np.unique(categ2subgroup)
    for subgroup in subgroup_ids:
        subgroups[subgroup] = {'y_pred':[], 'y_true':[], 'losses':[]}

    for i in range(len(y_true)):
        categ = y_true[i]
        subgroup = categ2subgroup[categ]
        subgroups[subgroup]['y_true'].append(y_true[i])
        subgroups[subgroup]['y_pred'].append(y_pred[i])
        subgroups[subgroup]['losses'].append(losses[i])
...
def consolidateTask(trainer, task, ngpus=None, consolidateModel=False, dataParallel=True):
    
    # Modify Classifier
    for key in list(trainer.dataset.task_to_num_classes.keys()):
        if task!=key:
            num_classes_for_other_task = trainer.dataset.task_to_num_classes[key]
    num_new_classes = num_classes_for_other_task + 1
    
    if consolidateModel:
        in_features = trainer.model.classifier[-1].in_features
        trainer.model.classifier[-1] = torch.nn.Linear(in_features, num_new_classes)
        if dataParallel:
    ...
    # Reassign Labels
    consolidatedSamples = []
    for sample in trainer.dataset.samples: # sample = (path to image, label)
        if sample[1] >= num_classes_for_other_task:
            consolidatedSamples.append((sample[0], num_classes_for_other_task))
        else:
            consolidatedSamples.append((sample[0], sample[1]))
    trainer.dataset.samples = consolidatedSamples
