...
class Config(object):
    ...
    def _get_num_classes(self, task):
        ...
        num_classes = utl.tools_new.get_num_classes(data_dir, islist=islist)
        return num_classes
    ...
    def get_model(self, ngpus, pretrained=False, epoch=-1, dataParallel=False):
        ...
        model = utl.tools_new.get_model(name=self._model_type, 
        ...
        if pretrained:
            restore_path = utl.tools_new.get_checkpoint(epoch=epoch, checkpoints_dir=self._checkpoints_dir) 
            if ngpus > 0:
...
class Trainer(object):
    ...
    def __call__(self, x, y, update_weights=True, activations=False, gradients=False, debug=None): # DEBUG
        ...
        prec_1, prec_5 = utl.tools.precision(output=output, target=y, topk=(1,5))
        prec_1 /= len(output)
        prec_5 /= len(output)             
...
class Validator(object):
    ...
    def __call__(self, x, y, activations=False, gradients=False, debug=None): # DEBUG
        ...
        if debug:
            ...
            utl.metrics.log_memory(debug) # DEBUG
        return loss.item(), prec_1, prec_5, output


def train(config_file, pretrained=False, restore_epoch=-1,epochs=50,
          valid_freq=1,save_freq=10,workers=1,ngpus=1,notebook=False, 
    ...
    for epoch in range(start_epoch, stop_epoch):
        ...
        if force_save or save_epoch(epoch):
            ...
            utl.tools_new.save_checkpoint(ckpt_data, config.checkpoints_dir, max_count=max_count, keep=keep_epochs)
...
def train_split(config_file, pretrained=False, restore_epoch=-1, epochs=50,
    ...
    for epoch in range(start_epoch, stop_epoch + 1):
        ...
        if ((epoch % save_freq) == 0):
            ...
            utl.tools_new.save_checkpoint(ckpt_data, config.checkpoints_dir, max_count=max_count)
...
def eval(y_true, y_pred):
    acc = mtrx.accuracy_score(y_true, y_pred)
    bal_acc = mtrx.balanced_accuracy_score(y_true, y_pred)
    confusion_matrix = mtrx.confusion_matrix(y_true, y_pred)
    report = mtrx.classification_report(y_true, y_pred)
...
class ImageNetTrain(object):
    ...
    def __call__(self, frac_epoch, inp, target):
        ...
        top1, top5 = utl.tools.precision(output, target, topk=(1, 5))
        top1 /= len(output)
        top5 /= len(output)
        lr = self.lr.get_lr()[0]
    ...
    def __call__(self):
        ...
        with torch.no_grad():
            for (inp, target) in tqdm.tqdm(self.data_loader, desc=self.name):
                ...
                p1, p5 =utl.tools.precision(output, target, topk=(1, 5))
                top1 += p1
                top5 += p5
...
def get_base_predictions(validator,ngpus):
    ...
    accuracy = mtrx.accuracy_score(y_true=y_true, y_pred=y_pred)
...
def get_match_samples(validator, match_accuracy, ngpus, verbose=False):
    ...
    if match_accuracy > accuracy: # need to increase accuracy by getting rid of errors
        ...
        new_accuracy = mtrx.accuracy_score(y_true=new_y_true, y_pred=new_y_pred)
